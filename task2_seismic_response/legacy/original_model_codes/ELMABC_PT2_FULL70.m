% ============================================================
% ELMABC_PT2_FULL70.m
% FULL 70 cases, PT2 style (ELMABC)
...
% OUTPUT ROOT
outRoot = 'E:\University\My Thesis\Flac model\New\Maku_PT2_Data\ELMABC_FULL70_OUT';
...
abc_max_iter         = 1000;
abc_pop_size         = 30;
abc_onlooker_count   = 20;
abc_max_acceleration = 0.4;
...
%% ============================================================
% ELM init + ABC optimize Win & b (NOT outputWeight)
% ============================================================
...
[lb, ub] = scalar_bounds_from_x0(x0, bound_scale, bound_cap);
problem.vars = ypea_var('x', 'real', 'size', nnn, 'lower_bound', lb, 'upper_bound', ub);

problem.obj_func = @(sol) rmse_elm_ridge(sol.x(:), inputtrainn_fit, targettrainn_fit, k, hln, activationFunction, lambda_ridge);

alg = ypea_abc();
alg.max_iter         = abc_max_iter;
alg.pop_size         = abc_pop_size;
alg.onlooker_count   = abc_onlooker_count;
alg.max_acceleration = abc_max_acceleration;
alg.display          = false;

best_sol = alg.solve(problem);
x = best_sol.solution.x(:);

elm.inputWeight         = vec2mat(x(1:numel(x1)), k)'; 
elm.biasOfHiddenNeurons = vec2mat(x(numel(x1)+1:end), hln);
...
%% Stats xlsx
stats_xlsx = fullfile(caseDir, sprintf('ELMABC_%s%s_n%d_Statistics_%s_failmode_%s.xlsx', ...
