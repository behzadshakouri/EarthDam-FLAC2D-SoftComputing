...
cd([QOIn '\n' num2str(nsample) '\']);
    ...
    hline = refline([1 0]);
    hline.Color = 'k';
%     minmin=0.9*min(min(min(FDM_ELMdata(:,18)),min(FDM_ELMABCdata(:,18))),min(min(FDM_ELMACORdata(:,18)),min(FDM_ELMIGWOdata(:,18))));
%     maxmax=1.1*max(max(max(FDM_ELMdata(:,18)),max(FDM_ELMABCdata(:,18))),max(max(FDM_ELMACORdata(:,18)),max(FDM_ELMIGWOdata(:,18))));
